...
var createElement = function createElement(tag) {
  ...
  classes.length ? (_node$classList = node.classList).add.apply(_node$classList, _toConsumableArray(classes)) : noop();
...
function upload(selector, options) {
  ...
  var uploadBtn = createElement('button', ['btn', 'btn-primary'], 'Upload');
  ...
  var openFileInputWindow = function openFileInputWindow() {
    return input.click();
  };

  var loadFilesHandler = function loadFilesHandler(event) {
  ...
  var removeFileHandler = function removeFileHandler(event) {
    ...
    var block = event.target.closest('.preview-img');
  ...
  var uploadFilesHandler = function uploadFilesHandler() {
    if (!files.length) {
      return;
    }
  ...
  openBtn.addEventListener('click', openFileInputWindow); // Triggers, when file/files are chosen

  input.addEventListener('change', loadFilesHandler); // Triggers on preview click -> handling image removing

  preview.addEventListener('click', removeFileHandler); // Triggers, when user sends files
...
(0, _upload.upload)('#file', {
  ...
  onUpload: function onUpload(files, blocks) {
    files.forEach(function (file, index) {
      var ref = storage.ref("user-images/".concat(file.name));
      var task = ref.put(file);
      task.on('state_changed', function (snapshot) {
        var loadPercentage = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0) + '%';
        ...
        return console.log('Error with Firebase:', error);
      }, function () {
